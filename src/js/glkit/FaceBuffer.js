...
import { GLFaceBuffer } from './gl/GLFaceBuffer'
...
export class FaceBuffer extends Buffer {
  ...
  *
  * @param {number} f1 - Face data to push into this face buffer.
  * @param {number} f2 - Face data to push into this face buffer.
  * @param {number} f3 - Face data to push into this face buffer.
  *
  ...
  * @param {number} f1 - Face data to set into this face buffer.
  * @param {number} f2 - Face data to set into this face buffer.
  * @param {number} f3 - Face data to set into this face buffer.
  *
  * @return {FaceBuffer} This face buffer for chaining purpose.
  */
  set (index, f1, f2, f3) {
    ...
    this._buffer[index] = f1
    this._buffer[index + 1] = f2
    this._buffer[index + 3] = f3
  ...
  */
  map (mapper, thisArg) {
    ...
    for (let index = 0; index < this._size; ++index) {
      this.set(
        index,
        ...mapper.call(thisArg, this.get(index), index, self)
      )
    }
